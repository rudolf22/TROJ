function [rho_mixture, out, phi, Z] = real_weight_density(env,sub)
%% WEIGHT_DENSITY
% 
...
%  See also: DENSITY_ELEMENT, MOLAR_DENSITY, DENSITY, MOLAR_MASS.
%  
%  Example:
%  
%  sub{1}.id = get_id('CH4');
        ...
        if (((sub{k}.id >= 1 ) & (sub{k}.id <= 308 )))
                ...
                if (T >= table_of_temperatures{3,sub{k}.id+1})
                    ...
                    if a == Inf
                        ...
                        rho{k} = roots([a*b -a (R*T+p*b) -p]).*molar_mass(sub(k));
                        v = size(rho{k},1);
                        if ((real(rho{k}(1:v))>=0))
                ...
                elseif ( T <= table_of_temperatures{3,sub{k}.id+1} )  & ( T >= table_of_temperatures{2,sub{k}.id+1} )
                    if (sub{k}.id>=1 & sub{k}.id<=118) & (isequal(table_of_elements.Table.Row(sub{k}.id).Cell{12,1},"Liquid"))
                        ...
                        [rho_l,out_l] = get_weight_density_l(env,sub);
                        rho{k}=rho_l{k}.*molar_mass(sub(k));
                        sub_phase{k} = 'l';
...
for k = subs_phase_found
    if (isequal(sub_phase{k},'g'))
    Z{k} = (p*molar_mass(sub(k)))/(rho{k}*R*T);
    men(k) = Z{k}*sub{k}.x;
    else
    Z{k} = Inf;
    men(k) = Inf;
...
for k = subs_phase_found
    if (isequal(sub_phase{k},'g'))
    phi{k} = (Z{k}*sub{k}.x)/men_sum;
    rho_mixture(k) = rho{k}*phi{k};
    else
    phi{k} = Inf;
    rho_mixture(k) = rho{k}*(sub{k}.w); 
    end
end
rho_mixture = sum(rho_mixture);
%% OUTPUTS
